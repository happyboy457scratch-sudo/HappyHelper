...
chatForm.addEventListener('submit', async (event) => {
  ...
  let apiKey = localStorage.getItem('studymate_openai_api_key') || '';
  if (!apiKey) {
    const typedKey = window.prompt('Enter your OpenAI API key (sk-...) to continue:');
    apiKey = (typedKey || '').trim();
    if (!apiKey) {
      addMessage('bot', 'No API key entered, so I cannot call ChatGPT yet.');
      return;
    }
    localStorage.setItem('studymate_openai_api_key', apiKey);
  }
...
popupCloseBtn.addEventListener('click', () => { loginPopup.hidden = true; });

addMessage('bot', 'Hi! I can help with KS3, KS4 and UK/US exam board questions. Ask your first question. Suggestion: ask me to create flash cards.');
